...
class MonthSummaryPrinter
  ...
  def print(year, month)
    ...
    entry.day_entries.each do |day, minutes|
      ...
      puts "%s %s - %s" % [day_of_week_abbr, day, minutes_to_time(minutes)]
    end

    puts "Days worked this month: %d" % entry.days_worked
    puts "Total count of work hours: %s/%s" % [minutes_to_time(entry.time_in_month), minutes_to_time(entry.days_worked * 8 * 60)]
